export class ASCIIGlobe {
  ...
  private characters = ' .·:+=#@█';  setRotation(rotation: number) {
    ...
    const theta = (lng + rotation) * Math.PI / 180;const x = this.radius * Math.sin(phi) * Math.cos(theta) * this.zoom;
...
return { x, y: tiltedY, z: tiltedZ };  }  private project3DTo2D(x: number, y: number, z: number): { x: number; y: number; visible: boolean } {
    const screenX = Math.round(x + this.width / 2);
    const screenY = Math.round(-y + this.height / 2);
    const visible = z > -this.radius * 0.8 * this.zoom;return { x: screenX, y: screenY, visible };  }  clear() {
    ...
    for (let lat = -90; lat <= 90; lat += 2) {
      for (let lng = -180; lng <= 180; lng += 2) {
        ...
        const { x: screenX, y: screenY, visible } = this.project3DTo2D(x, y, z);    if (visible && screenX >= 0 && screenX < this.width && screenY >= 0 && screenY < this.height) {
...
return false;  }  private isCoastline(lat: number, lng: number): boolean {
    if (!this.isDetailedLandmass(lat, lng)) return false;// Check if any neighbor is water
...
return false;  }  private getElevation(lat: number, lng: number): number {
    // Himalayas
    ...
    // Andes
    if (lat > -55 && lat < 10 && lng > -75 && lng < -65) return 0.8;return 0.2;  }  drawMarker(lat: number, lng: number, symbol: string, highlight: boolean = false) {
    const { x, y, z } = this.latLngTo3D(lat, lng, this.rotation);
    const { x: screenX, y: screenY, visible } = this.project3DTo2D(x, y, z);if (visible && screenX >= 0 && screenX < this.width && screenY >= 0 && screenY < this.height) {
...
return { visible, screenX, screenY };  }  drawConnection(lat1: number, lng1: number, lat2: number, lng2: number) {
    const steps = 30;
    for (let i = 0; i <= steps; i++) {
      const t = i / steps;  // Great circle interpolation
